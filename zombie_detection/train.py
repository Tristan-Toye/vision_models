...
Usage:
    python -m zombie_detection.train --model heatmap_cnn --config zombie_detection/config.yaml
"""
...
def compute_mean_iou_matched(
    pred_boxes: np.ndarray,
    gt_boxes: np.ndarray,
    iou_threshold: float = 0.5,
) -> float:
    """Mean IoU over greedy-matched pairs (IoU >= threshold).

    Uses the same greedy matching order as :func:`compute_precision_iou`.
...
def train_yolo_or_detr(
    ...
    from zombie_detection.models import get_model
...
def _plot_curves(history: dict, output_dir: Path, model_name: str):
    """Save train-loss and val-precision curves as PNG."""
    ...
    ax2.set_title(f"{model_name} - Val Precision")
    ax2.legend()
