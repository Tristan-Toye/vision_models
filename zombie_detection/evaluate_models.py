...
from zombie_detection.train import (
    train_model,
    compute_precision_iou,
    compute_mean_iou_matched,
    load_config,
)
...
def _evaluate_checkpoint(
    ...
    for group in cfg["experiments"]["distortion_eval_groups"]:
        ...
        for file_level, obs_path, box_path in tqdm(
            ...
            mean_ious.append(compute_mean_iou_matched(pred_boxes, gt_boxes))

        distortion_results[group_name] = {
            "avg_precision": np.mean(precisions) if precisions else 0.0,
            "avg_iou_matched": np.mean(mean_ious) if mean_ious else 0.0,
...
def _predict_fasterrcnn(
    ...
    if ckpt.exists():
        model.load_state_dict(torch.load(str(ckpt), map_location="cpu"))
    pred = model.predict(image_proc)  # xywh in image_proc pixel coordinates

    # Convert back to screen pixel coordinates (dataset GT is stored in screen coords).
    if pred.size == 0:
        return pred
    img_h, img_w = image_proc.shape[:2]
    screen_w = cfg["screen"]["width"]
    screen_h = cfg["screen"]["height"]
    if img_w > 0 and img_h > 0 and (img_w != screen_w or img_h != screen_h):
        sx = screen_w / float(img_w)
        sy = screen_h / float(img_h)
        ...
        if "best" in test_results or "last" in test_results:
            for ckpt_tag in ("best", "last"):
                ...
                for group_name, group_data in ckpt_data.items():
                    if isinstance(group_data, dict):
                        ...
                        row[f"mean_iou_{ckpt_tag}_{group_name}"] = group_data.get("avg_iou_matched", 0)
        else:
            ...
            for group_name, group_data in test_results.items():
                if isinstance(group_data, dict):
                    ...
                    row[f"mean_iou_best_{group_name}"] = group_data.get("avg_iou_matched", 0)
...
def main():
    ...
    parser.add_argument(
        "--test-only",
        action="store_true",
        help="Skip training; only (re-)run test evaluation for experiments with existing checkpoints.",
    )
    ...
    # ── Dataset check ──
    if args.test_only:
        ok = _check_split_exists(cfg, "test")
    else:
        ok = _check_dataset_exists(cfg)

    if not ok:
        if args.generate_dataset and not args.test_only:
            ...
            if args.test_only:
                print("Test-only mode requires only the test split to exist at:")
                print(f"  {data_dir / 'test'}")
    ...
    if args.test_only:
        # Only keep experiments with checkpoints. We'll (re-)compute test_results for them.
        experiments = [e for e in experiments if _experiment_has_checkpoint(Path(args.output_dir) / e["id"])]
    ...
    for i, exp in enumerate(exp_bar):
        ...
        if args.test_only:
            ...
            exp_bar.write(f"\n  TEST-ONLY {exp['id']}")
            exp_bar.write(
                f"        model={exp['model']}  loss={exp['loss']}  "
            ...
            # Preserve any existing train metadata if present
            result = {
                **exp,
                "status": "success" if "error" not in test_results else "failed",
                "train_time": 0,
                "train_results": {},
            ...
            failed_experiments.append(exp["id"]) if result["status"] == "failed" else None
